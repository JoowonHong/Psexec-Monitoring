...
IP1_3rd = int(Client_IP_1.split('.')[0])
IP1_4th = int(Client_IP_1.split('.')[-1])

IP2_3rd = int(Client_IP_2.split('.')[0])
IP2_4th = int(Client_IP_2.split('.')[-1])

if IP1_3rd!=IP2_3rd:
    print("Not Same 3rd IP")

for i in range(IP1_4th,IP2_4th+1):
    IP_4th_LIST.append(i)
...
def print_GPU_Every_Second(IP_4th):
    global df
    global IP1_3rd
    IP_3rd = IP1_3rd
    # global IP1_4th
    while True and not stop_event.is_set():
        ...
        psexec_cmd = f'{psexec_path} \\\\10.82.{IP_3rd}.{IP_4th} {command1}'
        ...
        GPU = Output_List[Utilization_idx+1:Utilization_idx+5:2]
...
############ 
# for i in IP_4th_LIST:

threads = []
for i in IP_4th_LIST:
    t = threading.Thread(target=print_GPU_Every_Second,args=(f"{i}",))
    t.start()
    threads.append(t)


# print_thread = threading.Thread(target=print_GPU_Every_Second,args=(f"{IP_4th_LIST[0]}",))
# print_thread_1 = threading.Thread(target=print_GPU_Every_Second,args=(f"{IP_4th_LIST[1]}",))
# print_thread_2 = threading.Thread(target=print_GPU_Every_Second,args=(f"{IP_4th_LIST[2]}",))
...
# while print_thread.is_alive() and print_thread_1.is_alive() and print_thread_2.is_alive():
#     time.sleep(0.1)
